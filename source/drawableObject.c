...
bool		drawProgressBarObject(void *self) {
	progressbar_t* t = (progressbar_t*)self;
	if (t->isHidden) return true;
	drawSprite(t->sprite);
	if(!(t->rectangle->draw(t->rectangle))) goto error;
	return true;
error:
	return false;
}
...
void deleteProgressBar(progressbar_t* bar) {
	if (!bar) return;
	deleteRectangle(bar->rectangle);
	deleteSprite(bar->sprite);
	free(bar);
}

progressbar_t *newProgressBar(sprite_t *sprite, sprite_t *fillsprite, float posX, float posY, float width, float height) {
	progressbar_t* ret;
	ret = (progressbar_t *)calloc(1, sizeof(progressbar_t));
	if (!ret) goto error;
	ret->isHidden = false;
	ret->sprite = sprite;
	ret->rectangle = newRectangle(posX, posY, width, height, 0, fillsprite);
	ret->draw = drawProgressBarObject;
	return ret;
error:
	return NULL;
}